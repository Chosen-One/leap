...
//				System.out.println(gesture.type()+" "+gesture.id()+" "+gesture.toString());
////				 switch (gesture.type()) {
////			        case TYPE_CIRCLE:
////			        	count+=1;
...
////			        case TYPE_SWIPE:
////			        	count+=1;
////			        	System.out.println("Swipe "+count);
////			            break;
////			        default:
...
class SampleListener extends Listener {
	...
	public void onConnect(Controller controller) {
		...
		System.out.println("Screen Resolution: X: " + screenWidth + ", H: " + screenHeight);
	...
	public void onFrame(Controller controller) {
		...
		if (readyForControl && rightHand.confidence() > .15) {
			if (!isMoving && !wasInTabState && frame.hands().count() > 1) {
				...
				if (leftHand.pinchStrength() > .8 && rightHand.pinchStrength() > .8) {
					if (!isResizing) {
				...
				if (isMoving) {
					...
					if (palmN.getX() != 0 && palmN.getY() != 0 && palmN.getZ() != 0) {
						if (palmN.getY() < -.1 && palmN.getZ() > -.8) {
							...
							System.out.println(
									"Normal: X: " + palmN.getX() + ", Y: " + palmN.getY() + ", Z: " + palmN.getZ());
							robot.keyPress(KeyEvent.VK_ALT);
							robot.keyPress(KeyEvent.VK_SPACE);
							robot.keyRelease(KeyEvent.VK_SPACE);
							robot.keyRelease(KeyEvent.VK_ALT);
			...
			if (!isMoving && !isResizing) {
				if (palmN.getY() < -.8 && palmN.getZ() > -.5) {
				...
				} else if (palmN.getY() >= .8 && wasFacingDown && !wasInTabState) {
					...
					robot.keyRelease(KeyEvent.VK_ALT);
					try {
						Runtime.getRuntime().exec("cmd /c start " + "C:\\WindowSwitcher.lnk");
					} catch (IOException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
					robot.delay(300);
				} else if (wasInTabState && !wasFacingDown && !wasTabbing && palmN.getY() < .45) {
					wasTabbing = true;
				} else if (wasInTabState && !wasFacingDown && wasTabbing && palmN.getY() > .75) {
					robot.keyPress(KeyEvent.VK_TAB);
					robot.keyRelease(KeyEvent.VK_TAB);
					wasTabbing = false;
				}
			 ...
			 * rightHand.grabStrength() < .1){ System.out.println(
			 * "Palm vertical velocity: " + rightHand.palmVelocity().getY());
			 * //float resultVerticalV =
			 ...
			 * }else{
			 */ if (!isMoving && !wasInTabState && frame.gestures().count() > 0 && frame.hands().count() == 1
					&& !isResizing) {
				CircleGesture circleGesture = new CircleGesture(frame.gestures().get(0));
				// System.out.println("Pinch strength: " +
				// rightHand.pinchStrength());
				if (circleGesture.durationSeconds() > .5 && !justCircleGestured && rightHand.pinchStrength() > .8) {
				...
				// System.out.println("xSpeed: " + xSpeed + ", ySpeed: " +
				// ySpeed);
				robot.mouseMove((int) (currentMouseX + xSpeed), (int) (currentMouseY - ySpeed)); // }
			}
		}
	}
}
